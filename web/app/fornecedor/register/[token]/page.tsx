...
export default function FornecedorRegisterPage() {
  ...
  async function registrar() {
    ...
    setSending(true);
    try {
      const res = await fetch(`/api/fornecedor/invite/${token}`, {
        method: "POST",
    ...
    return (
      <div className="min-h-screen bg-[var(--background)] flex items-center justify-center p-4">
        <div className="rounded-2xl border border-[var(--accent)]/40 bg-[var(--accent)]/10 p-8 max-w-sm w-full text-center shadow-sm">
          ...
          <p className="text-[var(--accent)]/90 text-sm mb-6">Faça login para acessar o painel do fornecedor.</p>
          <button
            onClick={() => router.replace("/fornecedor/login")}
    ...
    <div className="min-h-screen bg-[var(--background)] flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center">
          ...
          <p className="text-[var(--muted)] text-sm">Criar acesso de fornecedor</p>
        </div>
