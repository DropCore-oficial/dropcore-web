/**
 * POST /api/fornecedor/mensalidades/[id]/cobranca-pix
...
export async function POST(
  ...
  try {
    ...
    if (!email && process.env.MERCADOPAGO_TEST_MODE === "true") {
      email = "test@testuser.com"; // Modo teste aceita sem email
    }
    if (!email) {
      return NextResponse.json({ error: "E-mail não encontrado. Atualize sua conta para pagar via PIX." }, { status: 400 });
    }
