...
/** Botão ? discreto; painel abre ao passar o mouse (hover). `side=above` evita cobrir o card de cupom. */
function HelpBubble({
  open,
  onOpen,
  onClose,
  ...
  const posY =
    side === "above"
      ...
      : "top-[calc(100%+4px)]";
  const posX = align === "end" ? "right-0 left-auto" : "left-0";
    ...
    >
      ...
      {open && (
        <div
          id={tooltipId}
          role="tooltip"
          className={`absolute z-[80] ${posY} ${posX} w-[min(calc(100vw-2rem),17.5rem)] rounded-lg border border-neutral-200 bg-white p-3 text-left text-[11px] leading-relaxed text-neutral-700 shadow-md dark:border-neutral-600 dark:bg-neutral-800 dark:text-neutral-200`}
        >
          {children}
        </div>
      )}
