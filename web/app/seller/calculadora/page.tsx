...
import { useState, useEffect, useCallback, useRef, useId, useLayoutEffect, type ReactNode } from "react";
...
/** Botão ? discreto; painel em portal + fixed para não ser cortado por overflow. Hover no botão ou no painel. */
function HelpBubble({
  open,
  onOpen,
  onClose,
  ...
  const tooltipNode =
    ...
    createPortal(
      <div
        ...
        className="rounded-lg border border-neutral-200 bg-white p-3 text-left text-[11px] leading-relaxed text-neutral-700 shadow-lg dark:border-neutral-600 dark:bg-neutral-800 dark:text-neutral-200 pointer-events-auto"
...
export default function SellerCalculadoraPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (json.fornecedor_conectado_id) {
        setFornecedorConectadoId(json.fornecedor_conectado_id);
        setSelectedFornecedorId(json.fornecedor_conectado_id);
      }
    })();
    return () => {
      cancelled = true;
                ...
                >
                  ...
                  {fornecedores.map((f) => (
                    <option key={f.id} value={f.id}>
                      {f.nome}
                      {fornecedorConectadoId === f.id ? " (conectado)" : ""}
                    </option>
                  ))}
                </select>
              </Row>
                  ...
                  >
                    ...
                    {produtos.map((p) => (
                      <option key={p.id} value={p.id}>
                        {p.nome_produto || p.sku} — {BRL.format(p.custo)}
                      </option>
                    ))}
