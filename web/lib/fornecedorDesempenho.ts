...
/** Dados para gráfico/analytics do fornecedor (valor_fornecedor por dia, totais, top produto). */
export async function loadFornecedorDesempenho(
  orgId: string,
  fornecedorId: string,
  modo: string,
  periodoParam: string
): Promise<FornecedorDesempenho> {
  ...
  const lista = pedidos ?? [];
  const toKey = (d: Date) => d.toLocaleDateString("en-CA", { timeZone: TZ });
