 ...
 * Card inteiro (incluindo a foto) leva pro produto — é navegação de catálogo, não uma
 * ferramenta de zoom de foto (isso existe em telas de gestão, não na vitrine do seller).
 * Zoom leve na foto + elevação do card no hover. */
export function SellerCatalogoProdutoCard({ fornecedorId, grupo }: { fornecedorId: string; grupo: GrupoPreview }) {
  ...
  return (
    <Link
      ...
      className="group block rounded-2xl border border-[var(--card-border)] bg-[var(--card)] p-3 shadow-sm transition-all hover:-translate-y-[1px] hover:border-emerald-300 hover:shadow-md dark:hover:border-emerald-700"
    >
