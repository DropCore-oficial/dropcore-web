...
export function MensalidadeCardPaymentBrick({
  context,
  mensalidadeId,
  amount,
  payerEmail,
  ...
  const containerId = `mp-card-brick-${context}-${mensalidadeId}`;
  ...
  useEffect(() => {
    ...
    if (!payerEmail?.trim()) {
      onErroRef.current(
        "E-mail do pagador não encontrado. Atualize o e-mail no cadastro ou na conta de login.",
      );
      return;
    }
    if (amount <= 0) return;
            ...
            onSubmit: (cardFormData: unknown) => {
              ...
              return (async () => {
                try {
                  ...
                  const apiBase =
                    context === "seller" ? "/api/seller/mensalidades" : "/api/fornecedor/mensalidades";
                  const res = await fetch(`${apiBase}/${mensalidadeId}/cobranca-cartao`, {
                    method: "POST",
                    headers: {
                  ...
                  if (json.status === "approved" || json.mensalidade_liberada) {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      const msg =
        "O formulário de cartão demorou para carregar. Recarregue a página (Cmd+Shift+R). Se persistir, confira NEXT_PUBLIC_MERCADOPAGO_PUBLIC_KEY no .env.local.";
      setSdkErro(msg);
      onErroRef.current(msg);
      setCarregandoBrick(false);
    }, 20_000);
  ...
  }, [publicKey, payerEmail, amount, mensalidadeId, context, containerId]);
  ...
  if (!publicKey) {
    return (
      ...
      >
        Cartão indisponível: configure{" "}
        <code className="text-[11px]">NEXT_PUBLIC_MERCADOPAGO_PUBLIC_KEY</code> no{" "}
        <code className="text-[11px]">.env.local</code> (mesmo par de credenciais do access token).
      </p>
    );
  }
  ...
  if (!payerEmail?.trim()) {
    return (
      ...
      >
        Informe um e-mail no cadastro da empresa ou na conta de login para pagar com cartão.
      </p>
    );
  }
  ...
  return (
    <div className="mt-3">
      <p className="mb-2 text-xs text-neutral-600 dark:text-neutral-400">
        Preencha os dados do cartão abaixo. O pagamento é processado com segurança pelo Mercado Pago.
      </p>
